...
export function AlertBanner({ variant = "default", title, message, dismissible, onDismiss }: AlertBannerProps) {
  const icons = {
    default: Info,
  ...
  return (
    <Alert variant={variant}>
      ...
      {dismissible && onDismiss && (
        <button onClick={onDismiss} className="absolute right-2 top-2 rounded-md p-1 hover:bg-black/5">
          <X className="h-4 w-4" />
        </button>
      )}
    </Alert>
